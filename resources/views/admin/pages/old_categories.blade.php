@extends('admin.admin') @section('title','Categories') @section('description','myDescription to categories info') @section('keywords','myKeyWords to Categories info') @section('admin_main_content')
    ...
    <div class="row">
        <div class="col-12 col-lg-6">
            ...
            <form method="post" action="{{ route('cat.store') }}" class="form">
                ...
                <div class="form-group">
                    <input type="text" class="form-control  d-inline mr-sm-1 w-50" id="cat_name"
                           placeholder="Назва категорії" name="name" required value={{ old( 'name') }}>
                    <select class="selectpicker mr-sm-1" name="parent_id" required>
                        <option selected value="">Категорії...</option>
                        @foreach ($cats as $cat)
        ...
        <div class="col-12 d-none" id="edit-form">
            ...
            <form method="post" action="{{ route('cat.update') }}" class="form">
                ...
                <div class="form-group">
                    <input type="text" class="form-control  d-inline mr-sm-1 w-50" id="new-cat-name"
                           placeholder="Назва категорії" name="name" required>
            ...
            <table class="table table-striped">
                ...
                @foreach ($cats as $cat)
                    <tr>
                        ...
                        <td class="text-right">
                            <a href="{{ route('cat.delete',$cat->id) }}" class="btn btn-danger"
                               onclick="return confirm('Ви впевнені?')"><i class="fas fa-trash-alt"></i></a>
                            <a href="#edit-form" class="btn btn-info change-category" id={{ $cat->id }}
                                    data-name="{{ $cat->name }}"
